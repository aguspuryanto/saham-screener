...
import { WatchlistOutput } from '../../../domain/models/Watchlist';
...
export type WatchlistScreenerStatus = 'idle' | 'loading' | 'ok' | 'no-history' | 'insufficient-bars';
...
export function useWatchlistScreener() {
  const [results, setResults] = useState<Map<string, WatchlistOutput | null>>(new Map());
  const [statusByTicker, setStatusByTicker] = useState<Map<string, WatchlistScreenerStatus>>(new Map());
  ...
  const run = useCallback(async (stocks: Stock[]) => {
    ...
    await runWithConcurrency(stocks, CONCURRENCY_LIMIT, async (stock) => {
      ...
      if (!entry.ok || entry.bars.length === 0) {
        ...
        const output = computeWatchlistOutput(stock, entry.bars);
        setStatusByTicker((prev) =>
